!function($, $ele, $i) {
    ...
    $app.Manager = function() {
        ...
        function addButton() {
            ...
            }), $sl.find(".tp_templates_add_button").length || $sl.find(TP_FIND_SELECTOR).before($tpOpenLibraryButton)
        }

        function initAddButton($el) {
            var $t = $el.find(TP_FIND_SELECTOR);
            $t.length && !$el.find(".tp_templates_add_button").length && $t.before($tpOpenLibraryButton), $el.on("click.onAddElement", ".elementor-editor-section-settings .elementor-editor-element-add", addButton)
        }
        ...
        $ele.on('document:loaded', function() {
            console.log('Document loaded');
            var $previewContents = $ele.$previewContents;

            // Listen for the click event on the add section button
            $previewContents.on('click', '.elementor-editor-element-add', function() {
                console.log('Add section button clicked');
                // Add the custom button after a section is added
                setTimeout(function() {
                    console.log($previewContents.find('.elementor-add-section'));
                    $previewContents.find('.elementor-add-section').each(function(){
                        if($(this).find('.tp_templates_add_button').length == 0){
                            $(this).find(TP_FIND_SELECTOR).before($tpOpenLibraryButton);
                        }
                    });
                }, 100); // Delay to ensure the section is added to the DOM
            });
        });
        ...
        TP_FIND_SELECTOR = ".elementor-add-new-section .elementor-add-section-drag-title", 
        $tpOpenLibraryButton = '<div class="elementor-add-section-area-button tp_templates_add_button"> <img src="'+tpcoreEditor.templateLogo+'" /> </div>', 
        $tpResponsiveMap = {
            desktop: "100%",
            tab: "768px",
            mobile: "360px"
